...
run() {
  for seed in 1234 2345 3456; do
    for target_names in "C" "C++" "C#" "Java" "Go" "PHP" "Python" "VB"; do
      #preprocess data
      NAME=$(date +%Y%m%d%H)_MultilingualTrans_many_to_one_seed${seed}_lr${lr}_maxlen1536_warmup200_${source_names}_to_${target_names}
      OUTPUT_DIR=output/saved_models/MultilingualTrans/many_to_one_${lr}_maxlen1536_seed${seed}/${NAME}
      echo ${NAME}
      mkdir -p ${OUTPUT_DIR}
